 ...
 * Блок центральных кнопок действий над нижним меню в точном стиле Tile Family (Primer/1.jpg)
 * Кнопка "Помощь" со стрелкой ⬇️ и большая зеленая кнопка "Уровень 1" / "Спасти"
 */
...
export const RescueActionBar: React.FC<RescueActionBarProps> = ({
  ...
  return (
    <div className="relative w-full px-3 py-2 flex items-center justify-between gap-3 z-30 select-none">
      {/* 1. Левая кнопка: Золотая плашка "Помощь" с прыгающей стрелкой (Primer/1.jpg) */}
      <div className="relative flex-1">
        {/* Желтая прыгающая стрелочка вниз */}
        ...
        <button
          ...
          className={`group relative w-full h-15 rounded-2xl p-1 bg-gradient-to-b from-amber-200 via-amber-400 to-amber-600 border-2 border-amber-100 shadow-[0_6px_20px_rgba(245,158,11,0.5),0_0_15px_rgba(251,191,36,0.3)] transition-all active:scale-95 ${
            isRescueActive ? 'ring-2 ring-white scale-[1.02]' : 'hover:scale-[1.01]'
          }`}
        >
          {/* Красный бейдж уведомления в правом верхнем углу */}
          ...
          <div className="w-full h-full rounded-xl bg-gradient-to-b from-amber-100 via-amber-200 to-amber-300 flex items-center px-2 gap-2 border-t border-white/80 shadow-inner">
            {/* Круглый портрет замерзающей Золушки */}
            <div className="w-10 h-10 rounded-full bg-amber-400 p-0.5 shadow-md shrink-0 border border-amber-500 overflow-hidden">
              <img
      ...
      {/* 2. Правая кнопка: Сочная зеленая 3D кнопка "Уровень 1" / "Спасти" (Primer/1.jpg) */}
      <div className="flex-1">
        ...
        >
          <div className="w-full h-full rounded-xl bg-gradient-to-b from-emerald-400 via-emerald-500 to-emerald-600 flex items-center justify-center border-t border-emerald-200/80 shadow-inner">

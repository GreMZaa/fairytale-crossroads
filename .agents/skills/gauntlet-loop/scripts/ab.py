#!/usr/bin/env python3
"""Set up a blind A/B pair for one Gauntlet round.
...
Text artifacts (a spec, a page of copy, a module) go through the same path:
...
Usage:
  ab.py --round .gauntlet/<slug>/rounds/07 --ref ref/cod-hangar.png \\
        --cand .gauntlet/<slug>/rounds/07/artifact.png [--shrink 1000] [--squint]
"""
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Blind A/B pair for one Gauntlet round")
    ap.add_argument("--round", required=True, help="round directory, e.g. .gauntlet/x/rounds/07")
